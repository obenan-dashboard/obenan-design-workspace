...
`../START_HERE.md`, `../AGENTS.md`, `../STATUS.md`, `../LIBRARY_MAP.md`,
`README.md`, and, for Claude Code, `../CLAUDE.md`.
...
- PM `knowledge/product/README.md` (section index for the unified dated product
  evidence; load only the matching numbered sections from
  `knowledge/product/UI_UX_KNOWLEDGE_DOC.md`)
- CUI `knowledge/implementation/README.md` (implementation-source bridge to
  the private Common UI package; authorized access required for live source)
...
| Product or dashboard UX and UI | DC | PM section index (load only its matching numbered sections); CUI for implementation; ADG sections 4 and 5; TCC sections 5 to 7; DP Parts III and V | Immutable private source when restricted account or security detail is essential; DRIFT D04, D09, D12, D13; PARITY; LEDGER section 5 | BS; messaging research; Gleb archive; unselected PM sections | Dated map and Common UI package require current re-verification; DDR DR-D05, DR-D06, DR-D08, DR-D09, DR-D11, DR-D12 | PARTIAL; STOP for a collapsed or misrepresented claim state, a safety or accessibility defect, or a new/replacement component whose Common UI contract could not be verified |
| Onboarding and conversion flows | DC; MC | ADG sections 3 and 4; AMG (free-value, demo, conversion); TCC (Field, Button, Progress); CUI if implemented | PARITY (onboarding nodes); R07; R08 | R02, R03, R04; Gleb archive | DDR DR-D06, DR-D12; MDR DR-04, DR-05; current Common UI contract if implemented | PARTIAL; STOP for a capability claim without a claim record or an unverified replacement component |
...
| Token or component implementation | DC; CUI | TCC (all); DDR DR-D05 to DR-D12; DRIFT; PARITY | LEDGER sections 3 and 5; whole-brain read-only | Messaging research; brand PDFs | Current Common UI package and relevant export must be verified; DDR DR-D11, DR-D05 to DR-D09, DR-D10; exact authorized repository ceiling per `../AGENTS.md` | PARTIAL (aliases and candidates, no final values); STOP without an exact authorized repository and path ceiling or before creating a replacement for an unverified Common UI contract |
